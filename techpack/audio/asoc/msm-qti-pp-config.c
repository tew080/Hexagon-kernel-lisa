...
static int msm_qti_pp_asphere_send_params(int port_id, int copp_idx, bool force)
{
	...
	if (set_strength) {
		...
		ret = q6common_pack_pp_params(packed_params +
					      ...
					      &param_size);
		if (ret) {
			pr_debug("%s: Failed to pack params for audio sphere"
				" strength, error %d\n", __func__, ret);
			goto done;
		}
			...
			struct snd_ctl_elem_value *ucontrol)
{
	...
	if (ucontrol->value.integer.value[0] < 0 ||
		...
		pr_debug("%s: invalid ffecns effect value %ld\n",
			__func__, ucontrol->value.integer.value[0]);
		return -EINVAL;
	}
	...
	if (ret)
		pr_debug("%s: failed to set %s for ffecns\n",
