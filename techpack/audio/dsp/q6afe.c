...
static void q6afe_unload_avcs_modules(u16 port_id, int index)
{
	...
	if (ret < 0)
		pr_debug("%s: avcs module unload failed %d\n", __func__, ret);
		 ...
		 uint32_t use_case, u32 format_id)
{
	...
	if (num_modules <= 0) {
		pr_debug("%s: Invalid number of modules to load\n", __func__);
		return -EINVAL;
	}
	...
	for (i = 0; i < MAX_ALLOWED_USE_CASES; i++) {
		if (pm[i] == NULL) {
			...
			if (use_case == ENCODER_CASE) {
				...
				pr_debug("%s:load usecase %d not supported\n",
					 __func__, use_case);
				ret = -EINVAL;
				goto fail;
			}
...
load_unload:
	...
	if (i == MAX_ALLOWED_USE_CASES) {
		pr_debug("%s: Not enough ports available\n", __func__);
		return ret;
	}
fail:
	kfree(pm[i]->payload);
				    ...
				    uint32_t payload_size)
{
	...
	case AFE_PARAM_ID_CALIB_RES_CFG_V2:
		...
		if (num_ch > SP_V2_NUM_MAX_SPKRS) {
			pr_debug("%s: Error: num_ch %d is greater than expected\n",
				__func__,num_ch);
			return -EINVAL;
		}
		if (param_hdr.param_size != (sizeof(struct asm_calib_res_cfg) - ((2 - num_ch) * 4))) {
			pr_debug("%s: Error: param_size %d is greater than expected\n",
...
/* for mius start */
		...
		if (NULL != data->payload) {
			...
			pr_debug("[EXPORT_SYMBOLLUS]: payload ptr is Invalid");
#endif
/* for mius end */
		...
		if (wait) {
			...
			if (!ret) {
...
#ifdef AUDIO_FORCE_RESTART_ADSP
				pr_debug("%s: DSP returned error[%s][%d]\n", __func__,
					adsp_err_get_err_str(atomic_read(
				...
				if (err_count >= ADSP_ERR_LIMITED_COUNT) {
					err_count = 0;
					pr_debug("%s: DSP returned error more than limited, restart now !\n", __func__);
...
static int afe_send_cps_config(int src_port)
{
	...
	if ((-1 == this_afe.vi_tx_port) || (!this_afe.cps_ch_mask) ||
	    (!this_afe.cps_config)) {
		pr_debug("%s: speaker prot not configured for 0x%x\n", __func__,
		       src_port);
		return -EINVAL;
	}
		...
		union afe_spkr_prot_config *prot_config, uint32_t param_size)
{
	...
	if (ret)
		pr_debug_ratelimited("%s: port = 0x%x param = 0x%x failed %d\n",
...
int afe_send_cdc_dma_data_align(u16 port_id, u32 cdc_dma_data_align)
{
	...
	if (ret)
		pr_debug("%s: AFE cdc cdc data alignment for port 0x%x failed %d\n",
...
static int afe_send_hw_delay(u16 port_id, u32 rate)
{
	...
	if (ret)
		pr_debug_ratelimited("%s: AFE hw delay for port 0x%x failed %d\n",
		       __func__, port_id, ret);
...
static int afe_port_topology_deregister(u16 port_id)
{
	...
	if (ret < 0) {
		pr_debug("%s: get AVS build versions failed %d\n",
		       __func__, ret);
		goto done;
	}
	...
	if (build_major_version != AVS_BUILD_MAJOR_VERSION_V2 ||
		...
		ret = -EINVAL;
		pr_debug("%s: AVS build versions mismatched %d\n",
		       __func__, ret);
		goto done;
	}
...
int afe_send_port_power_mode(u16 port_id)
{
	...
	if (ret) {
		pr_debug("%s: AFE port[%d] get power mode is invalid!\n",
			__func__, port_id);
		return ret;
	}
	...
	if (ret) {
		pr_debug("%s: AFE set power mode enable for port 0x%x failed %d\n",
			__func__, port_id, ret);
		return ret;
	}
			...
			struct afe_ttp_config *ttp_cfg)
{
	...
	ret = q6afe_pack_and_set_param_in_band(port_id,
					       ...
					       (u8 *) &ttp_gen_enable);
	if (ret) {
		pr_debug("%s: AVS_DEPACKETIZER_PARAM_ID_TTP_GEN_STATE for port 0x%x failed %d\n",
			__func__, port_id, ret);
		goto exit;
	}
	...
	ret = q6afe_pack_and_set_param_in_band(port_id,
					       ...
					       (u8 *) &ttp_gen_cfg);
	if (ret)
		pr_debug("%s: AVS_DEPACKETIZER_PARAM_ID_TTP_GEN_CFG for port 0x%x failed %d\n",
			__func__, port_id, ret);
exit:
	return ret;
}
			...
			u16 afe_in_channels, u16 afe_in_bit_width)
{
	...
	if (format == ASM_MEDIA_FMT_LC3) {
		if (cfg->data.lc3_dec_config.dec_codec.from_Air_cfg
			...
			if (ret) {
				pr_debug("%s: CAPI_V2_PARAM_LC3_DEC_MODULE_INIT for port 0x%x failed %d\n",
				       __func__, port_id, ret);
				goto exit;
			}
			...
			if (ret) {
				pr_debug("%s: AVS_COP_V2_PARAM_ID_STREAM_INFO for port 0x%x failed %d\n",
				       __func__, port_id, ret);
				goto exit;
			}
		}
				 ...
				 u32 scrambler_mode, u32 mono_mode)
{
	...
	if (format == ASM_MEDIA_FMT_AAC_V2) {
		...
		if (frame_size_ctl_value_v2 > 0) {
			...
			ret = q6afe_pack_and_set_param_in_band(port_id,
					...
					(u8 *) &frame_ctl_param);
			if (ret) {
				pr_debug("%s: AAC_FRM_SIZE_CONTROL with VBR support failed %d\n",
					__func__, ret);
				goto exit;
			}
		}
	...
	if (format == ASM_MEDIA_FMT_LC3) {
		...
		if (ret) {
			pr_debug("%s: CAPI_V2_PARAM_LC3_ENC_INIT for port 0x%x failed %d\n",
				__func__, port_id, ret);
			goto exit;
		}
			    ...
			    struct afe_ttp_config *ttp_cfg)
{
	...
	ret = afe_get_power_mode(port_id, &power_mode);
	if (ret)
		pr_debug("%s: AFE port[0x%x] get power mode is invalid!\n",
			__func__, port_id);
	...
	if (ret) {
		pr_debug_ratelimited("%s: AFE enable for port 0x%x failed %d\n",
			__func__, port_id, ret);
		goto fail_cmd;
	}
	...
	if ((codec_format != ASM_MEDIA_FMT_NONE) &&
		...
		if (enc_cfg != NULL) {
			...
			if ((q6core_get_avcs_api_version_per_service(
				...
				if (ret < 0) {
					pr_debug("%s:encoder load for port 0x%x failed %d\n",
						__func__, port_id, ret);
					goto fail_cmd;
				}
			}
			...
			if ((q6core_get_avcs_api_version_per_service(
				...
				if (codec_format == ENC_CODEC_TYPE_LDAC ||
					...
					ret = q6afe_load_avcs_modules(2, port_id,
						DECODER_CASE, codec_format);
				if (ret < 0) {
					pr_debug("%s:decoder load for port 0x%x failed %d\n",
						__func__, port_id, ret);
					goto fail_cmd;
				}
			}
		...
		if (ttp_cfg != NULL) {
			ret = q6afe_send_ttp_config(port_id, *afe_config,
						    ttp_cfg);
			if (ret) {
				pr_debug("%s: AFE TTP config for port 0x%x failed %d\n",
					 __func__, port_id, ret);
				goto fail_cmd;
			}
		}
...
static int afe_sidetone_iir(u16 tx_port_id)
{
	...
	if (size > MAX_SIDETONE_IIR_DATA_SIZE) {
		pr_debug("%s: iir_config size is out of bounds:%d\n", __func__, size);
		mutex_unlock(&this_afe.cal_data[cal_index]->lock);
		ret = -EINVAL;
		goto done;
	}
...
int afe_close(int port_id)
{
	...
	if (q6core_get_avcs_api_version_per_service(
		...
		if (afe_spk_ramp_dn_cfg(port_id) < 0)
			pr_debug("%s: ramp down config failed\n", __func__);
	}
			  ...
			  uint32_t clk_reset)
{
	...
	ret = q6core_get_avcs_avs_build_version_info(
			&build_major_version, &build_minor_version,
						&build_branch_version);
	if (ret < 0) {
		pr_debug("%s error in retrieving avs build version %d\n",
				__func__, ret);
		return ret;
	}
	...
	afe_api_version = q6core_get_avcs_api_version_per_service(
				APRV2_IDS_SERVICE_ID_ADSP_AFE_V);
	if (afe_api_version < 0) {
		pr_debug("%s error in retrieving afe api version %d\n",
	...
	if (ret) {
		pr_debug("%s: cannot get clk src name for port id 0x%x\n",
			__func__, port_id);
		return -EINVAL;
	}
	...
	if ((build_major_version == AVS_BUILD_MAJOR_VERSION_V2) &&
		...
		ret = q6afe_svc_pack_and_set_param_in_band(index, param_hdr,
						   (u8 *) &clk_drift);
		if (ret < 0)
			pr_debug_ratelimited("%s: AFE PLL clk drift failed with ret %d\n",
				    __func__, ret);
	} else {
		ret = -EINVAL;
		pr_debug_ratelimited("%s: AFE PLL clk drift failed ver mismatch %d\n",
				    __func__, ret);
	}
				   ...
				   uint32_t mclk_freq)
{
	...
	if (dyn_mclk_cfg.enable) {
		...
		if (ret) {
			pr_debug_ratelimited("%s: mclk cb during enable failed %d\n",
					   __func__, ret);
			return ret;
		}
	...
	ret = q6afe_svc_pack_and_set_param_in_band(index, param_hdr,
						   (u8 *) &dyn_mclk_cfg);
	if (ret < 0) {
		pr_debug_ratelimited("%s: ext MCLK clk cfg failed with ret %d\n",
				   __func__, ret);
		mutex_unlock(&this_afe.afe_cmd_lock);
		goto stop_mclk;
	}
	...
	if (!dyn_mclk_cfg.enable) {
		...
		ret = afe_ext_mclk.ext_mclk_cb(afe_ext_mclk.private_data,
				dyn_mclk_cfg.enable, mclk_freq, &dyn_mclk_cfg);
		if (ret) {
			pr_debug_ratelimited("%s: mclk cb during disable failed %d\n",
					   __func__, ret);
			return ret;
		}
	}
				   ...
				   uint32_t mux_val)
{
	...
	if ((build_major_version != AVS_BUILD_MAJOR_VERSION_V2) ||
		...
		pr_debug("%s: AFE_PARAM_ID_CLOCK_MUX_CFG unsupported by AVS\n", __func__);
		return -EINVAL;
	}
	...
	ret = q6afe_svc_pack_and_set_param_in_band(IDX_RSVD_3, param_hdr,
						   (u8 *) &clk_mux_cfg);
	if (ret < 0) {
		pr_debug_ratelimited("%s: ext mclk mux cfg failed with ret %d\n",
				   __func__, ret);
	}
	mutex_unlock(&this_afe.afe_cmd_lock);
	return ret;
							...
							uint32_t mclk_freq)
{
	...
	if (afe_ext_mclk.ext_mclk_cb) {
		ret =  afe_ext_mclk.ext_mclk_cb(afe_ext_mclk.private_data,
			dyn_mclk_cfg->enable, mclk_freq, dyn_mclk_cfg);
		if (ret) {
			pr_debug_ratelimited("%s: get mclk cfg failed %d\n",
					__func__, ret);
			return ret;
		}
	} else {
		pr_debug_ratelimited("%s: mclk callback not registered\n", __func__);
		return -EINVAL;
	}
...
int afe_set_lpass_clock_v2(u16 port_id, struct afe_clk_set *cfg)
{
	...
	if (clkinfo_per_port[idx].mclk_src_id != MCLK_SRC_INT) {
		...
		if ((build_major_version != AVS_BUILD_MAJOR_VERSION_V2) ||
			...
			pr_debug("%s: ext mclk not supported by AVS\n", __func__);
			return -EINVAL;
		}
		...
		u16 port_id)
{
	...
	if (ret) {
		pr_debug("%s: Failed to get Tmax Xmax logging data\n", __func__);
		goto get_params_fail;
	}
		...
		struct afe_sp_v4_th_vi_calib_resp *calib_resp)
{
	...
	if (th_vi_ca_state < FBSP_INCORRECT_OP_MODE ||
		...
		pr_debug("%s: invalid fbsp state %d\n", __func__, th_vi_ca_state);
		goto get_params_fail;
	}
			...
			u32 ch_mask)
{
	...
	if (!cps_config) {
		pr_debug("%s: cps config is NULL\n", __func__);
		return;
	}

	if (q6audio_validate_port(src_port) < 0) {
		pr_debug("%s: Invalid src port 0x%x\n", __func__, src_port);
		return;
	}
...
void afe_set_lsm_afe_port_id(int idx, int lsm_port)
{
	if (idx < 0 || idx >= MAX_LSM_SESSIONS) {
		pr_debug("%s: %d Invalid lsm port index\n", __func__, idx);
		return;
	}
...
int afe_paired_rx_tdm_port_ops(int intf_idx, bool enable, atomic_t *dai_group_ref)
{
	...
	if (enable) {
		...
		if (atomic_read(group_ref) == 0) {
			...
			if (rc < 0) {
				pr_debug("%s:Failed to enable paired rx afe group 0x%x\n"
				 , __func__, group_id);
				return -EINVAL;
			}
		}
		if (atomic_read(port_ref) == 0){
			...
			if (rc < 0) {
				...
				pr_debug("%s: fail to open paired rx AFE port 0x%x\n", __func__, port_id);
				return -EINVAL;
			} else {
		...
		if (atomic_read(port_ref) == 1) {
			...
			if (rc < 0) {
				pr_debug("%s:Failed to close paired rx port 0x%x", __func__, port_id);
				return -EINVAL;
			} else {
			...
			if (atomic_read(group_ref) == 0) {
				...
				if (rc < 0) {
					pr_debug("%s: fail to disable paired AFE group 0x%x\n", __func__, group_id);
					return -EINVAL;
				}
			}
			...
			pr_debug("%s:No need to close port",__func__);
			return -EINVAL;
		}
	}
	return 0;
