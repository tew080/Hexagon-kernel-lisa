...
int hh_vcpu_populate_affinity_info(u32 cpu_idx, u64 cap_id)
{
	...
	if (!is_vcpu_info_populated) {
		if (populate_need) {
			...
			hh_cpumap[nr_vcpus].pcpu = cpu_logical_to_phys(selected);
			hh_cpumap[nr_vcpus].curr_pcpu = cpu_logical_to_phys(selected);
						...
						unsigned long cmd, void *data)
{
	 ...
	 */
	if (cmd == HH_RM_NOTIF_VM_STATUS  && vm_status == HH_RM_VM_STATUS_READY) {
		if (populate_need && is_tuivm(vm_status_payload->vmid))
			hyp_core_ctl_isolate_cpus();
	} else if (cmd == HH_RM_NOTIF_VM_STATUS &&
