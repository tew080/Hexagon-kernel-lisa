...
static void teo_update(struct cpuidle_driver *drv, struct cpuidle_device *dev)
{
	...
	for (i = 0; i < drv->state_count; i++) {
		...
		if (target_residency <= cpu_data->sleep_length_ns) {
			idx_timer = i;
			if (target_residency <= measured_ns)
				idx_duration = i;
		}
	}
		      ...
		      bool *stop_tick)
{
	...
	if (!dev->states_usage[0].disable) {
		idx = 0;
		if (drv->states[1].target_residency > duration_ns)
			goto end;
	}
	...
	for (i = 1; i < drv->state_count; i++) {
		...
		if (s->target_residency > duration_ns)
			break;
