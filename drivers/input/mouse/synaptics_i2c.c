...
static irqreturn_t synaptics_i2c_irq(int irq, void *dev_id)
{
	...
	mod_delayed_work(system_power_efficient_wq, &touch->dwork, 0);
...
static void synaptics_i2c_work_handler(struct work_struct *work)
{
	 ...
	 */
	mod_delayed_work(system_power_efficient_wq, &touch->dwork, delay);
}
...
static int synaptics_i2c_open(struct input_dev *input)
{
	...
	if (polling_req)
		mod_delayed_work(system_power_efficient_wq, &touch->dwork,
